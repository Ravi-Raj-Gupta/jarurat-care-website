...
export const load: PageServerLoad = async ({ locals, params }) => {
	...
	if (error || !article) {
		throw redirect(303, '/cms/doctor-dashboard/articles');
	}

	// Ensure the user is the author (optional check, but good for security)
	if (article.author_id !== session.user.id && profile.role !== 'super_admin' && profile.role !== 'cms_admin') {
		throw redirect(303, '/cms/doctor-dashboard/articles');
	}
