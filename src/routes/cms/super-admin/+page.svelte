<script lang="ts">
	...
	$: totalShares = Number(analytics.totalShares ?? analytics.shares ?? 0);
	$: dailyActiveUsersValue = Array.isArray(analytics.dailyActiveUsers)
		? analytics.dailyActiveUsers.reduce(
				(sum: number, item: any) =>
					sum + Number(item.users ?? item.count ?? item.value ?? 0),
				0
			)
	...
	// Overview helpers (derived only, no server changes)
	$: pendingVerificationCount = users.filter(
		(u: any) => u.role === 'Doctor' && u.verification_status !== 'approved'
	).length;

	$: recentContent = publishedContent.slice(0, 6);

	$: topPublishingDoctors = [...publishingDoctors]
		.sort((a: any, b: any) => (b.articles || 0) - (a.articles || 0))
		.slice(0, 5);
		...
		<div class="page-content">
			...
			{#if activeSection === 'overview'}
				...
				<div class="section-heading">
					<div>
						...
						<p>A quick snapshot of platform activity and content.</p>
					</div>
				</div>
				...
				<div class="overview-grid">
					...
					<div class="stat-card">
						<div class="stat-icon purple"><Stethoscope size={20} /></div>
						<div>
					...
					<div class="stat-card">
						<div class="stat-icon green"><FileText size={20} /></div>
						<div>
							<span>Published Content</span>
							<strong>
								{formatNumber(
									(analytics.totalArticles || 0) + (analytics.totalResearchPapers || 0)
								)}
							</strong>
						</div>
					</div>
					...
					<div class="stat-card">
						<div class="stat-icon orange"><Clock size={20} /></div>
						<div>
							<span>Pending Verifications</span>
							<strong>{formatNumber(pendingVerificationCount)}</strong>
						</div>
					</div>
				...
				<div class="overview-two-column">
					...
					<div class="panel">
						<div class="panel-header">
							<div>
								<h3>Recent Published Content</h3>
								<p>Latest articles, research and CMS updates</p>
							</div>
							<button class="text-button" on:click={() => (activeSection = 'articles')}>
								View All
							</button>
						</div>
						...
						{:else}
							<div class="empty-small">No published content yet.</div>
						{/if}
					</div>
					...
					<div class="panel">
						<div class="panel-header">
							<div>
								<h3>Quick Stats</h3>
								<p>Overall engagement snapshot</p>
							</div>
						</div>
						...
						<div class="quick-stats">
							<div>
								...
								<strong>{formatNumber(analytics.totalViews)}</strong>
							</div>
							<div>
							...
							<div>
								...
								<strong>{formatNumber(analytics.totalLikes)}</strong>
							</div>
							<div>
								<span>CMS Content</span>
								<strong>{formatNumber(analytics.totalCMSContent)}</strong>
							</div>
						</div>
					</div>
				...
				<div class="panel">
					<div class="panel-header">
						<div>
							<h3>Top Publishing Doctors</h3>
							<p>Ranked by published articles and research</p>
						</div>
						<Star size={16} />
					</div>
						...
						<div class="author-list">
							{#each topPublishingDoctors as doctor, index}
								<div class="author-row">
									...
									<div class="author-avatar">{(doctor.name || 'D').charAt(0)}</div>
					...
					{:else}
						<div class="empty-small">No publishing activity yet.</div>
					{/if}
				</div>
			...
			{:else if activeSection === 'cms'}
				...
				<div class="panel">
					...
					<form method="POST" use:enhance={handleCMSCreate} action="?/createCMSContent" class="cms-create-form">
						...
						<input name="featuredImage" placeholder="Featured image URL" />
						...
						<textarea name="seoDescription" placeholder="SEO description"></textarea>
						<select name="status"><option value="draft">Save Draft</option><option value="published">Publish Now</option></select>
						<button class="action-button approve-button" type="submit">Create Content</button>
					</form>
				</div>
			...
			{:else if activeSection === 'community'}
				...
				<div class="section-heading">
					<div>
						<h2>Community</h2>
						<p>Browse all doctors and their community activity.</p>
					</div>
				</div>
				...
				<div class="panel">
					<div class="table-wrapper">
						<table>
							<thead><tr><th>Doctor</th><th>Specialization</th><th>Organization</th><th>Followers</th><th>Following</th><th>Content</th><th>Status</th></tr></thead>
							<tbody>
								{#each data.communityDoctors || [] as doctor}
									<tr>
										<td><div class="user-cell"><div class="user-avatar">{(doctor.name || 'D').charAt(0)}</div><div><strong>{doctor.name}</strong><span>{doctor.email || 'Doctor'}</span></div></div></td>
										<td>{doctor.specialization || 'General'}</td>
										<td>{doctor.organization || '—'}</td>
										<td>{formatNumber(doctor.followersCount)}</td>
										<td>{formatNumber(doctor.followingCount)}</td>
										<td>{formatNumber((doctor.articlesCount || 0) + (doctor.researchCount || 0))}</td>
										<td><span class={getStatusClass(doctor.verification_status)}>{doctor.verification_status || 'pending'}</span></td>
									</tr>
								{/each}
							</tbody>
						</table>
			...
			{:else if activeSection === 'analytics'}
				...
				<div class="section-heading">
					<div>
						...
						<p>
							Monitor content performance, reader activity and engagement across Jarurat Care.
						</p>
					</div>
				</div>
				...
				<div class="analytics-grid analytics-grid-primary">
					<div class="analytics-card">
						<div class="analytics-card-top">
							<div class="analytics-icon views"><Eye size={19} /></div>
							<span class="analytics-label">TOTAL VIEWS</span>
						</div>
						<strong>{formatNumber(analytics.totalViews)}</strong>
						<span class="analytics-description">Total views across published content</span>
					</div>

					<div class="analytics-card">
						<div class="analytics-card-top">
							<div class="analytics-icon visitors"><Users size={19} /></div>
							<span class="analytics-label">UNIQUE VISITORS</span>
						</div>
						<strong>{formatNumber(uniqueVisitors)}</strong>
						<span class="analytics-description">Distinct visitors from tracked sessions</span>
					</div>

					<div class="analytics-card">
						<div class="analytics-card-top">
							<div class="analytics-icon active"><TrendingUp size={19} /></div>
							<span class="analytics-label">DAILY ACTIVE USERS</span>
						</div>
						<strong>{formatNumber(dailyActiveUsersValue)}</strong>
						<span class="analytics-description">Active users from tracked activity</span>
					</div>

					<div class="analytics-card">
						<div class="analytics-card-top">
							<div class="analytics-icon likes"><Heart size={19} /></div>
							<span class="analytics-label">LIKES</span>
						</div>
						<strong>{formatNumber(analytics.totalLikes)}</strong>
						<span class="analytics-description">Article likes recorded</span>
					</div>

					<div class="analytics-card">
						<div class="analytics-card-top">
							<div class="analytics-icon saves"><Bookmark size={19} /></div>
							<span class="analytics-label">SAVES</span>
						</div>
						<strong>{formatNumber(analytics.totalSaves)}</strong>
						<span class="analytics-description">Articles saved by readers</span>
					</div>

					<div class="analytics-card">
						<div class="analytics-card-top">
							<div class="analytics-icon shares"><ArrowUpRight size={19} /></div>
							<span class="analytics-label">SHARES</span>
						</div>
						<strong>{formatNumber(totalShares)}</strong>
						<span class="analytics-description">Content shares recorded</span>
					</div>


					<div class="analytics-card">
						<div class="analytics-card-top">
							<div class="analytics-icon comments"><MessageCircle size={19} /></div>
							<span class="analytics-label">COMMENTS</span>
						</div>
						<strong>{formatNumber(analytics.totalComments)}</strong>
						<span class="analytics-description">Reader comments</span>
					</div>
				</div>
					...
					<div class="panel large-panel">
						<div class="panel-header">
							<div>
								...
								<p>Likes, saves and comments recorded over time</p>
							</div>
							<div class="legend">
								<span><i class="legend-dot likes-dot"></i>Likes</span>
								<span><i class="legend-dot saves-dot"></i>Saves</span>
								<span><i class="legend-dot comments-dot"></i>Comments</span>
							</div>
						</div>
								...
								{#each analytics.engagementTrend as day}
									<div class="trend-column">
										<div class="bars">
											<div
												class="bar likes-bar"
												style={`height:${Math.min(100, Number(day.likes || 0) * 12 + 8)}%`}
												title={`Likes: ${day.likes || 0}`}
											></div>
											<div
												class="bar saves-bar"
												style={`height:${Math.min(100, Number(day.saves || 0) * 12 + 8)}%`}
												title={`Saves: ${day.saves || 0}`}
											></div>
											<div
												class="bar comments-bar"
												style={`height:${Math.min(100, Number(day.comments || 0) * 12 + 8)}%`}
					...
					<div class="panel">
						<div class="panel-header">
							<div>
								...
								<p>Unique users active during tracked days</p>
							</div>
						</div>
								...
								{#each dailyActiveUsersTrend as day}
									<div class="dau-row">
										<div class="dau-row-top">
											...
											<strong>{formatNumber(day.users ?? day.count ?? day.value ?? 0)}</strong>
										</div>
										<div class="dau-progress">
											<div
												style={`width:${Math.min(100, (Number(day.users ?? day.count ?? day.value ?? 0) / Math.max(1, dailyActiveUsersValue)) * 100)}%`}
											></div>
										</div>
									</div>
								{/each}
						...
						{:else}
							<div class="chart-empty">
								<TrendingUp size={34} />
								<p>No daily active user events recorded yet.</p>
							</div>
						{/if}
					</div>
				</div>
				...
				<div class="panel">
					<div class="panel-header">
						<div>
							...
							<p>Published content ranked by views</p>
						</div>
					</div>
										...
										<tr>
											...
											<td>
												<div class="popular-content">
													<div class="popular-icon"><FileText size={16} /></div>
													<div>
														<strong>{item.title || 'Untitled'}</strong>
											...
											<td><span class="metric likes-metric"><Heart size={13} />{formatNumber(item.likes)}</span></td>
											<td><span class="metric saves-metric"><Bookmark size={13} />{formatNumber(item.saves)}</span></td>
										</tr>
									{/each}
								</tbody>
							</table>
							...
							{#if analytics.articlesByCategory?.length}
								{#each analytics.articlesByCategory as category}
									<div class="category-row">
										...
										<small>{category.percentage}% of published articles</small>
									</div>
								{/each}
							{:else}
								<div class="empty-small">No category data available.</div>
							{/if}
						</div>
					</div>
					...
					<div class="panel">
						...
						<div class="user-breakdown">
							<div class="breakdown-item">
								<div class="breakdown-icon reader"><Users size={18} /></div>
								<div class="breakdown-main">
									<span>Readers</span>
									<div class="progress"><div style={`width:${analytics.totalUsers ? (analytics.totalReaders / analytics.totalUsers) * 100 : 0}%`}></div></div>
								</div>
								<strong>{formatNumber(analytics.totalReaders)}</strong>
							</div>

							<div class="breakdown-item">
								<div class="breakdown-icon doctor"><Stethoscope size={18} /></div>
								<div class="breakdown-main">
									<span>Doctors</span>
									<div class="progress"><div style={`width:${analytics.totalUsers ? (analytics.totalDoctors / analytics.totalUsers) * 100 : 0}%`}></div></div>
								</div>
								<strong>{formatNumber(analytics.totalDoctors)}</strong>
							</div>

							<div class="breakdown-item">
								<div class="breakdown-icon admin"><ShieldCheck size={18} /></div>
								<div class="breakdown-main">
									<span>Admins</span>
									<div class="progress"><div style={`width:${analytics.totalUsers ? (analytics.totalAdmins / analytics.totalUsers) * 100 : 0}%`}></div></div>
								</div>
								<strong>{formatNumber(analytics.totalAdmins)}</strong>
							</div>
						</div>
					</div>
				</div>
...
<style>
	...
	.analytics-icon.views { background: #eff6ff; color: #2563eb; }
	.analytics-icon.visitors { background: #f5f3ff; color: #7c3aed; }
	.analytics-icon.active { background: #ecfdf5; color: #059669; }
	.analytics-icon.likes { background: #fef2f2; color: #ef4444; }
	.analytics-icon.saves { background: #fff7ed; color: #ea580c; }
	.analytics-icon.shares { background: #eef2ff; color: #4f46e5; }
	.analytics-icon.comments { background: #f0fdf4; color: #16a34a; }
	...
	.saves-dot { background: #f97316; }
	.comments-dot { background: #16a34a; }
	...
	.likes-bar { background: #ef4444; }
	.saves-bar { background: #f97316; }
	.comments-bar { background: #16a34a; }
	...
	.dau-row-top span { color: #64748b; }
	.dau-row-top strong { color: #0f172a; }
	...
	.user-breakdown { padding: 15px 20px 20px; }
	...
	.breakdown-icon.reader { background: #eff6ff; color: #2563eb; }
	.breakdown-icon.doctor { background: #ecfeff; color: #0891b2; }
	.breakdown-icon.admin { background: #faf5ff; color: #9333ea; }

	.breakdown-main { flex: 1; }
	...
	.breakdown-item:nth-child(2) .progress div { background: #0891b2; }
	.breakdown-item:nth-child(3) .progress div { background: #9333ea; }
	...
	.popular-table-wrapper { overflow-x: auto; }
	...
	.popular-table tbody tr:hover { background: #f8fafc; }
	...
	.likes-metric { color: #ef4444; }
	.saves-metric { color: #ea580c; }


	/* CATEGORIES */

	.category-list { padding: 16px 20px 20px; }

	.category-row { padding: 10px 0; }
	...
	.category-row-top span { color: #334155; font-weight: 600; }
	.category-row-top strong { color: #0f172a; }
	...
	.author-list { padding: 7px 20px 15px; }
	...
	.author-count span { font-size: 8px; color: #94a3b8; }
	...
	.role-form select, .cms-create-form input, .cms-create-form textarea, .cms-create-form select { border:1px solid #e2e8f0; border-radius:7px; padding:8px 9px; font-size:11px; background:#fff; color:#0f172a; }
	.cms-create-form { display:grid; grid-template-columns:repeat(2,minmax(0,1fr)); gap:10px; padding:18px 20px 20px; }
